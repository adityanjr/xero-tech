...
# Enter here your working week - Nighttime/Daytime directory
working_directory = r"C:\Users\saman\Desktop\Xero Tech\Data Annotation\Week 2\Loc 3\Night"

# Don't change this ( Used to create a New Folder)
target_directory = os.path.join(working_directory, "FilteredData")
...
video_files = [os.path.join(working_directory, "videos", x)
               for x in os.listdir(os.path.join(working_directory, "videos"))]
...
def video_extraction(video_add, video_name, target_folder, fps_rate=1, second_s=3):
    ...
    while Video.isOpened():
        ...
        if int(sec_frame_count % frames) == 0:
            if second_skip % 3 == 0:
                total_frame_count += 1
                cv2.imwrite(target_folder+"\\"+video_name+"_{}_{}_{}.jpg".format(
                    total_frame_count, total, sec_frame_count//frames), frame)
                second_skip += 1
            else:
                second_skip += 1
